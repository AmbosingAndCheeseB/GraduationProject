...
def model_ANN(x):
    ...
    H1_output_drop = tf.nn.dropout(H1_output, keep_prob)
    W2 = tf.Variable(tf.random_normal(shape=[hidden1_size, hidden2_size]))
    b2 = tf.Variable(tf.random_normal(shape=[hidden2_size]))
    H2_output = tf.nn.relu(tf.matmul(H1_output_drop, W2) + b2)
    H2_output_drop = tf.nn.dropout(H2_output, keep_prob)
    ...
    logit = tf.matmul(H2_output_drop, W_out) + b_out
